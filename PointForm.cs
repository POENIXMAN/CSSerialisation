...
namespace FormsApp
{
    public partial class PointForm : Form
    {
        ...
        private void btnSerialize_Click(object sender, EventArgs e)
        {
            ...
            dlg.Filter = "SOAP|*.soap|XML|*.xml|JSON|*.json|Binary|*.bin|YAML|*.yaml;*.yml";
                ...
                new FileStream(dlg.FileName, FileMode.Open, FileAccess.Read))
            {
                switch (Path.GetExtension(dlg.FileName))
                {
                    ...
                    case ".yml":
                        ...
                        {
                            var rawPoints = yamlDeserializer.Deserialize<List<Dictionary<string, int>>>(reader);

                            points = rawPoints.Select(dict =>
                            {
                                if (dict.ContainsKey("Z"))
                                    return new Point3D(dict["X"], dict["Y"], dict["Z"]);
                                else
